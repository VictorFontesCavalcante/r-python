...
fn check_function_call(name: Name, args: Vec<Expression>, env: &Environment) -> Result<Type, ErrorMessage> {
    let func = match env.get(&name) {
        Some(f) => f,
        None => return Err(format!("{} not found.", name))
    };

    if let Type::TFunction(params_type, retrn) = func {
        if args.len() != params_type.len() {
            return Err(format!("[Type Error] {} requires {} arguments, got {}", name, params_type.len(), args.len()));
        }
        for (arg, param) in args.iter().zip(params_type) {
            let arg_type = check(arg.clone(), env)?;
            if arg_type != *param {
                return Err(format!("[Type Error] {} expected type {:?}, got {:?}", name, param, arg_type));
            }
        }
        Ok(*retrn.clone())
    } else {
        Err(format!("[Type Error] cannot call {}", name))
    }
}
